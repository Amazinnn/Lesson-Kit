# Writing Knowledge Statements (quick_absorption)

## 一句话定位

quick_absorption 不是解释，是定位——告诉学生"这里有颗珍珠，它长这样，去源材料里把它拾起来"。
...
- **Advance Organizer** (Ausubel): A brief orienting statement before detailed content improves comprehension.
- **Cognitive Theory of Multimedia Learning** (Mayer): Avoid extraneous material. Every word in quick_absorption must serve either definition or through-line function — no filler.
...
- `templates/structure-plan-template.md`: The quick_absorption field
- `templates/lesson-template.md`: Rendering rules for quick_absorption
- `views/first-pass/skills/through-line-selection.md`: Provides the through-line perspective

## Multi-Angle Interpretation Patterns

- **Definition View**: State what KP is. Pattern: "[KP名] 是 [核心属性] 的 [类别]"
- **Operational View**: State what KP does. Pattern: "当 [条件] 时，用 [KP名] 来 [操作目标]"
- **Visual View**: Describe visual representation. Pattern: "在 [图表类型] 中，[KP名] 表现为 [视觉特征]"
- **Boundary View**: When KP does NOT apply. Pattern: "[KP名] 适用于 [场景]，但不适用于 [边界]"
- **Error-Misconception View**: Common error. Pattern: "常见错误：[错误]，因为 [KP名] 实际要求 [正确约束]"
