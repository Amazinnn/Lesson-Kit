...
> **2026-08-29 修订**：本文 §二 所述「合流今天列表 + 卡片轻会话」已在
> 实现会话中被所有者改判（DISCUSSION-RECORD 专题 19）：准备练习列表 =
> 选区视图、候选按需拉取（按钮带计数、一行一短语）、方向卡 UI 全拆、
> UI 只到知识点级。以 `openspec/changes/consolidate-practice-views/`
> 修订版为准；§一/§三/§四/§五 仍有效。
