...
目标在月历中的持续时间条：从开始日期延伸到截止日期；没有开始日期的旧目标退化为截止日上的单日条。同一时间重叠的目标上下分轨并列，跨周目标按自然周切段但仍表示同一个目标。它只呈现目标时间范围，不自动修改计划。
...
### 时间安排 / Time View
练习页中栏日计划之下的实验性只读视图：目标时间跑道月历（开始日→截止日、重叠目标分轨、今日高亮）与 14 天任务量柱状并排。重日（某天到期数 ≥ 非零日均 2 倍）标「重」，可一键预填重排请求到 Agent 输入框（不发送）。只读，不改任何数据。
_Avoid_：日历应用、自动重排
