...
- 已有题目原地可改（不换题号、不重导）：单题走 `data update problem`，批量走 `problem-patch` 清单（一次预检、一份备份、一个事务、逐条报错、记批次号 + **每行改前旧值**，`ingest rollback --batch` 原样写回）。可改描述性字段、`practice_modes`、整份 `micro_quiz`、来源三字段与 `answer_key`；不能改题号，难度仍走 `difficulty`，**不认识的字段名报错不静默丢弃**；改题面/解析/知识点/学科形态清空难度评级，改模式/载荷/来源不动评级。客观题题干上限 800 字。
