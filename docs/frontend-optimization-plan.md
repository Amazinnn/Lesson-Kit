...
- Agent 写入由 `lesson-kit data` 的显式命令治理；普通问答零写入。结构变化后允许保留 conversation id 的受控刷新。
...
## 二十二、v15 工作台侧栏与对话滚动（2026-08-27）

- 桌面三栏内侧边缘提供轻量拖拽调宽，限制在可读范围且不持久化；窄屏仍使用左右抽屉。
- Agent 聊天消息区改为受约束的可滚动 flex 区域，长对话可直接用鼠标滚轮浏览。
- 滑槽只改变当前统一力场并触发重热，不改变关系弹簧、节点净空、坐标、SQLite 或学习记录。

## v4.1 每日计划与 Agent 微调方向（2026-08-28）

练习页未来以“长期目标 / 阶段目标 / 今日短队列”作为同一学习入口。短队列保持整体性，只表达今天需要推进的知识范围和大致配量，不把内部算法拆成学生必须执行的微动作。

默认配量算法是稳定底座，Agent 负责理解自然语言反馈并微调；学生只看最终计划。日历和任务量曲线属于后续 Experimental 视图，不能反过来增加当前页面密度。Agent 的自动重算以每日首次打开为触发点，状态栏与停止入口保持在对话区顶部。

## v4.2 练习范围与计划纠偏（2026-08-28）

本轮实现必须把知识视图（知识图谱 / 知识列表）作为练习范围的唯一显式来源。勾选
知识点后通过具名 `practice-selected` 交接进入练习；Agent 只有在收到明确练习意图时
才能用新范围替换当前选择，普通对话不得改变选择。直接打开练习且没有范围时显示空
交接状态，不从弱项、全池或今日队列自动拉题。

练习启动卡要求且只要求一种 `exam`、`flash_card`、`yes_no` 模式，会话期间不得混用或
静默降级。Flash Card 与 Yes-No 仍是契约入口，只有内容数据显式标注并存在可用题目时
才可进入；本文不把它们描述为已经实现的功能。

长期目标和阶段目标只渲染真实目标卡（标题、覆盖进度、DDL，详情按需展开）；今日计划
是独立且最多三项的粗粒度队列。无目标时不生成虚构课程目标、学习时长或掌握度，队列
点击只预填显式范围并要求继续选择模式。选择、导航、草稿、跳题和查看计划保持零写入。
