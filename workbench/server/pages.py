...
def practice_page(workspace, workspaces, weak_items, plan=None, suggestions=None,
    ...
    middle = (
        ...
        "<label><input id='flash-direction-forward' type='radio' name='flash-direction' value='forward' checked> 正向 · 概念先行</label>"
        "<label><input id='flash-direction-reverse' type='radio' name='flash-direction' value='reverse'> 反向 · 阐释先行</label>"
        "<p>会话内方向固定：双向卡在反向会话按背面提问，单向卡始终按正面提问。</p></fieldset>"
...
def time_view_html():
    return (
        ...
        "<h2>时间安排</h2></div></div>"
        "<div id='time-view-content' class='time-view-content'>"
        "<section class='time-calendar' aria-labelledby='calendar-title'>"
        "<div class='time-module-heading'><div><p class='time-module-kicker'>目标跑道</p>"
        "<h3 id='calendar-title'>本月安排</h3></div>"
        "<span id='calendar-month-label' class='time-module-meta'></span></div>"
        "<div id='calendar-grid' class='calendar-grid'></div>"
        "</section>"
        "<section class='time-workload' aria-labelledby='workload-title'>"
        "<div class='time-module-heading'><div><p class='time-module-kicker'>复习节奏</p>"
        "<h3 id='workload-title'>未来 14 天复习负荷</h3>"
        "<p class='time-module-description'>每天到期、建议回看的学习项</p></div>"
        "<button id='workload-prefill' class='outline sm hidden' type='button'>让 Agent 看看</button>"
        "</div>"
        "<div class='workload-summary' aria-label='复习负荷摘要'>"
        "<strong id='workload-total'>共 0 项</strong>"
        "<span id='workload-peak'>峰值 —</span>"
        "<span id='workload-overdue'>逾期 0 项</span>"
        "</div>"
        "<div class='workload-chart-scroll'>"
        "<div id='workload-bars' class='workload-bars' role='list' aria-label='未来 14 天每日到期复习项'></div>"
        "</div>"
        "</section></div>"
