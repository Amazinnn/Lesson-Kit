...
def _prompt(message, context):
    ...
    return (
        ...
        "难度（可选题）：若你能判断，加 difficulty 取 1-5（1-2 记忆识别、2-4 直接应用、\n"
        "4-5 综合构造）并必须同时写一行 difficulty_basis 说明依据（例如「判断题型，只需\n"
        "回忆定义」）；两项必须成对出现，只写 difficulty 会被拒收。判断不了就两项都不写，\n"
        "不要猜。闪卡不填难度。\n"
        f"id 形如 {course}-{chapter}-fc-NNN 或 -mq-NNN（NNN 为三位数字；课程必须是\n"
        f"{course}，其他课程的 id 会被拒收；与池内已有 id\n"
