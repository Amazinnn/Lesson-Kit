...
The workbench SHALL offer an experimental, read-only time view inside the
review page: a month grid placing goals on their deadline dates (multiple
goals per day stack, never merged), today highlighted, and a 14-day bar curve
of due schedule rows per day (each direction counted as one item). Days whose
...
input. The view SHALL NOT reschedule, write, or alter any learning data, and
SHALL render honest empty states when there is nothing scheduled.
